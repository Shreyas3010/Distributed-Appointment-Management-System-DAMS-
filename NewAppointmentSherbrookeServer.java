...
public class NewAppointmentSherbrookeServer {
    static Properties props = null;
    
    public static void listenForRequestInterServer(AppointmentImplSherbrooke serverService, int serverUdpPort) throws InterruptedException, ParseException {
	...
	public static String processRequestFromUser(String response, AppointmentImplSherbrooke serverService) throws ParseException, InterruptedException {
			String reply="";
    ...
    public static void listenForRequestFromRM(AppointmentImplSherbrooke serverService)
    {
		...
		while(true){
			...
			try {
				...
				datagramSocketReplica=new DatagramSocket(FinalVariables.SHERBROOKE_SERVER_PORT);
				byte[] buffer=new byte[1000];
	            ...
	            DatagramPacket requestToRM = new DatagramPacket(messageToRM, messageToRM.length, datagramPacketFromRM.getAddress(), datagramPacketFromRM.getPort());

	            datagramSocketReplica.send(requestToRM);
	...
	{
		AppointmentImplSherbrooke.sherbrookeAppointmentDB = new HashMap<>();
		AppointmentImplSherbrooke.initializeDatabase();
		AppointmentImplSherbrooke appointmentImplSherbrooke =new AppointmentImplSherbrooke();
		ReplicaManager.setExpectedSequencerId(1);
		Runnable taskInterServer = () -> {
            try {
            	listenForRequestInterServer(appointmentImplSherbrooke, FinalVariables.SHERBROOKE_INTER_SERVER_PORT);
		...
		Runnable task = () -> {
            listenForRequestFromRM(appointmentImplSherbrooke);
        };
