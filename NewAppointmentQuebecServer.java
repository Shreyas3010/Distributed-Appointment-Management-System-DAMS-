...
public class NewAppointmentQuebecServer {

    static Properties props = null;
    
    public static void listenForRequestInterServer(AppointmentImplQuebec serverService, int serverUdpPort) throws InterruptedException, ParseException {
	...
	public static String processRequestFromUser(String response, AppointmentImplQuebec serverService) throws ParseException, InterruptedException {
		String reply="";
...
public static void listenForRequestFromRM(AppointmentImplQuebec serverService)
{
	...
	while(true){
		...
		try {
			...
			datagramSocketReplica=new DatagramSocket(FinalVariables.QUEBEC_SERVER_PORT);
			byte[] buffer=new byte[1000];
	...
	{
		AppointmentImplQuebec.quebecAppointmentDB = new HashMap<>();
		AppointmentImplQuebec.initializeDatabase();
		
		AppointmentImplQuebec appointmentImplQuebec =new AppointmentImplQuebec();
		ReplicaManager.setExpectedSequencerId(1);
        Runnable taskInterServer = () -> {
            try {
            	listenForRequestInterServer(appointmentImplQuebec, FinalVariables.QUEBEC_INTER_SERVER_PORT);
		...
		Runnable task = () -> {
            listenForRequestFromRM(appointmentImplQuebec);
        };
